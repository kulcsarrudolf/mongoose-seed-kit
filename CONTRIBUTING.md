...
Thanks for your interest in improving `mongoose-seed-kit`!
...
2. Install dependencies:
   ...
   yarn install
   ```
3. **Docker is required** to run the test suite — the integration tests spin up a real
   MongoDB instance via [Testcontainers](https://node.testcontainers.org/).

## Development workflow

- **Build** (CJS via tsup): `yarn build`
- **Test** (Mocha + Testcontainers): `yarn test`
- **Coverage** (c8): `yarn coverage`
- **Lint** (ESLint): `yarn lint` (autofix: `yarn lint:fix`)
- **Format** (Prettier): `yarn format` (check: `yarn format:check`)

A Husky pre-commit hook runs lint-staged (ESLint + Prettier); commit messages are validated
against [Conventional Commits](https://www.conventionalcommits.org/) via commitlint. The
Docker-backed test suite runs in CI rather than on pre-commit.
